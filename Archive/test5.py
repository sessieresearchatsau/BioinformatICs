...
def write_txt_file(edges_by_category, txt_filename="protein_interactions.txt"):
    """
    Writes a text file grouping edges by interaction type.
    Format example:
    
    PEPTIDE BONDS:
    {ALA12->ARG13,ARG13->ASN14}
    
    HYDROGEN BONDS:
    {ALA12->LYS20,VAL13->ASN40}
    ...
    """
    with open(txt_filename, "w") as f:
        for category, edges in edges_by_category.items():
            # Convert edges into a single line inside { ... } with commas
            # Each edge is (label1, label2), we want "label1->label2"
            connections = []
            for (a, b) in edges:
...
def write_html_file(pdb_text, html_filename="protein_view.html"):
    """
    Exports an HTML file with an interactive 3D model using py3Dmol.
    You can open this file in any modern browser to view the protein.
    """
    ...
    # _make_html() generates a standalone HTML string (with embedded JS/CSS)
...
def main():
    ...
    write_txt_file(edges_by_category, "protein_interactions.txt")
    print("[Info] Wrote interactions to 'protein_interactions.txt'")
    
    # 5) Create interactive HTML with py3Dmol
    write_html_file(pdb_data, "protein_view.html")
    print("[Info] Wrote interactive 3D view to 'protein_view.html'")
    print("Open 'protein_view.html' in your browser to explore the protein.")

# If you run it as a script (python script.py in VSCode):
