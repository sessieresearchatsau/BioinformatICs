...
def write_txt_file(uniprot_id, edges_by_category):
    ...
    plus an ALL INTERACTIONS section with all edges combined into one big set.
    """
    txt_filename = f"{uniprot_id}_interactions.txt"
    
    # We'll collect all edges in a set to avoid duplicates in the "ALL INTERACTIONS" section
    all_edges = set()
    ...
    with open(txt_filename, "w") as f:
        ...
        # Now, one big set "ALL INTERACTIONS"
        all_connections = []
...
def main():
    ...
    # 4) Write .txt with grouped interactions + ALL INTERACTIONS
